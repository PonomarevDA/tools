...
RGB565_FROM_COLOR = [
    ...
    dronecan.uavcan.equipment.indication.RGB565(red=31, green=61, blue=0),
    dronecan.uavcan.equipment.indication.RGB565(red=0, green=61, blue=31),
    dronecan.uavcan.equipment.indication.RGB565(red=31, green=61, blue=31),
]
...
class TestSolid:
    ...
    def test_random_color_after_restart():
        ...
        expected_color = RGB565_FROM_COLOR[TestSolid.color]

        assert compare_colors(recv_color, expected_color), f"{recv_color} != {expected_color}"
...
class TestBlinking:
    ...
    config = [
            ...
            Parameter(name=PARAM_LIGHTS_DUTY_CYCLE_PCT, value=0),
        ]
    @staticmethod
    def test_after_restart():
        ...
        enabled_colors_counter = 0
        disabled_colors_counter = 0
        unexpected_colors_counter = 0

        # Skip first enabled wave
        for _ in range(100):
            color = lights.recv_color()
            if compare_colors(color, enabled_color):
                enabled_colors_counter += 1
            elif compare_colors(color, disabled_color):
                disabled_colors_counter += 1
            else:
                unexpected_colors_counter += 1
            
        assert unexpected_colors_counter == 0, f"Got an unexpected color (need {enabled_color})"
        assert enabled_colors_counter > 0, "Enabled color never appeared"
        assert disabled_colors_counter > 0, "Disabled color never appeared"
...
class TestUiLedsMode:
    ...
    @staticmethod
    def test_ui_leds_mode():
        """
        When the autopilot is disarmed (ArmingStatus is Disarm or RawCommand is not >= 0)
        the node is controlled by the LightCommand command.
        """
        lights = LightsNode()
        ...
        for idx in range(128):
            ...
            for _ in range(2):
                color = lights.recv_color()
            assert color.red == idx%32 and color.green == idx%64 and color.blue == idx%32
